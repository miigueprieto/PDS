...
def filtro_homomorfico(imagen, frec_corte=30, a=0.5, b=1.5):
    ...
    plt.figure()
    plt.imshow(imagen, cmap='gray' if len(imagen.shape) !=3 else None)
    plt.title('Original')
    plt.figure()
    plt.imshow(img_final, cmap='gray' if len(imagen.shape) !=3 else None)
    plt.title('Filtro Homomórfico')
    plt.show()

filtro_homomorfico(skimage.data.moon())